...
| **rizin** / **rz-bin** | 0.7.1 (extracted from Cutter's AppImage) | Headless static disassembly (`rz-asm`), full-binary function analysis (`aaa`) that found 721 functions in the unpacked shellcode buffer, and import-table listing (`rz-bin -i`) on the reconstructed final payload. |
| **`file`** | — | Quick validation that a decrypted buffer is a real, well-formed PE before investing more analysis time in it. |
...
| **pycryptodome** | — | Independent verification of the layer-1 AES-256-CBC decryption outside the emulator, and later used to reproduce the layer-2 decryption entirely statically (no emulator involved) to recover the final payload — see [`analysis/05-final-payload-capabilities.md`](analysis/05-final-payload-capabilities.md). |
